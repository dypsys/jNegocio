...
class jNegocioModelCurrencies extends jFWModelCRUD {
    ...
    function _buildQueryWhere($query)
    {
        ...
        if ($filter)
        {
            ...
            $where = array();
            $where[] = '(LOWER(tbl.currency_id) LIKE '.$this->getDbo()->Quote($key).')';
            $where[] = '(LOWER(tbl.currency_name) LIKE '.$this->getDbo()->Quote($key).')';
            $where[] = '(LOWER(tbl.currency_code) LIKE '.$this->getDbo()->Quote($key).')';
