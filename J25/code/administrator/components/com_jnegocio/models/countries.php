...
class jNegocioModelCountries extends jFWModelCRUD 
{
	function _buildQueryWhere(&$query)
    {
		...
		if ($filter) {
			...
			$where = array();
			$where[] = '(LOWER(tbl.country_id) LIKE '.$this->_db->Quote($key).')';
			$where[] = '(LOWER(tbl.`'.$this->_lang->getField('name').'`) LIKE '.$this->_db->Quote($key).')';
			$where[] = '(LOWER(tbl.isocode_2) LIKE '.$this->_db->Quote($key).')';
			$where[] = '(LOWER(tbl.isocode_3) LIKE '.$this->_db->Quote($key).')';
        ...
        if ($filter_id) {
       		$query->where( 'tbl.country_id ='. $filter_id, 'AND' );
       	}
    }
