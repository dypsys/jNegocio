...
<script type="text/javascript">
	...
	{
		if (task == 'cancel' || document.formvalidator.isValid(document.id('taxrates-form'))) {
			Joomla.submitform(task, document.getElementById('taxrates-form'));
		} else {
			var msg = '<?php echo $this->escape(JText::_('JGLOBAL_VALIDATION_FORM_FAILED'));?>';
			if($('tax_rate').hasClass('invalid')){msg += '\n\n\t* <?= @JText::_( 'COM_JNEGOCIO_TAXRATE_NAME_LABEL' ); ?> <?= @JText::_( 'COM_JNEGOCIO_IS_INVALID' );?>';}
			alert(msg);
			return false;		
		}
	}
...
<form action="<?= @JRoute::_($this->action); ?>" method="post" id="taxrates-form" name="adminForm" class="form-validate" >
	<div class="width-60 fltlft">
		<fieldset class="adminform">
			<legend><?php echo empty($this->item->$tbl_key) ? JText::_('COM_JNEGOCIO_NEW_ITEM') : JText::_('COM_JNEGOCIO_EDIT_ITEM'); ?></legend>
			<ul class="adminformlist">
				<li>
			   		<label id="geozone_id-lbl" class="hasTip" title="<?= @JText::_( 'COM_JNEGOCIO_GEOZONE_NAME_LABEL' ); ?>::<?= @JText::_( 'COM_JNEGOCIO_GEOZONE_NAME_DESC' ); ?>" for="geozone_id">
						<?= @JText::_( 'COM_JNEGOCIO_GEOZONE_NAME_LABEL' ); ?>
					</label>
					<?= HelperSelect::geozones( @$this->row->geozone_id, 'geozone_id', '', 'geozone_id', false );?>
			   	</li>
			   	<li>
			   		<label id="typetax_id-lbl" class="hasTip" title="<?= @JText::_( 'COM_JNEGOCIO_TYPETAX_NAME_LABEL' ); ?>::<?= @JText::_( 'COM_JNEGOCIO_TYPETAX_NAME_DESC' ); ?>" for="typetax_id">
						<?= @JText::_( 'COM_JNEGOCIO_TYPETAX_NAME_LABEL' ); ?>
					</label>
					<?= HelperSelect::typetaxes( @$this->row->typetax_id, 'typetax_id', '', 'typetax_id', false, true );?>
			   	</li>
				<li>
					<label id="name-lbl" class="hasTip required" title="<?= @JText::_( 'COM_JNEGOCIO_TAXRATE_NAME_LABEL' ); ?>::<?= @JText::_( 'COM_JNEGOCIO_TAXRATE_NAME_DESC' ); ?>" for="name">
						<?= @JText::_( 'COM_JNEGOCIO_TAXRATE_NAME_LABEL' ); ?>
						<span class="star">&nbsp;*</span>
					</label>
					<input class="inputbox required" type="text" name="tax_rate" id="tax_rate" value="<?= @$this->row->tax_rate; ?>" size="7" maxlength="7" />					
				</li>
			</ul>
		</fieldset>
	</div>
