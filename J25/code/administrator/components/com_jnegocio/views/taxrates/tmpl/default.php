...
<form action="<?php echo JRoute::_( $this->action );?>" method="post" name="adminForm" id="adminForm">
	<fieldset id="filter-bar">
		...
		<div class="filter-select fltrt">
			<?= HelperSelect::geozones( @$state->filter_geozoneid, 'filter_geozoneid', array('class' => 'inputbox necFilter', 'size' => '1', 'onchange' =>'submitform( );' ), 'filter_geozoneid', true); ?>
			<?= HelperSelect::typetaxes( @$state->filter_geozoneid, 'filter_typetaxid', array('class' => 'inputbox necFilter', 'size' => '1', 'onchange' =>'submitform( );' ), 'filter_typetaxid', true); ?>
	...
	<thead>
		<tr>
			...
			<th class="title"><?php echo JText::_('COM_JNEGOCIO_GEOZONE_NAME_LABEL' ); ?></th>
			<th class="title"><?php echo JText::_('COM_JNEGOCIO_TYPETAX_NAME_LABEL' ); ?></th>
			<th class="title"><?php echo JText::_('COM_JNEGOCIO_TAXRATE_NAME_LABEL' ); ?></th>
		...
		<tr class="row<?= @$k; ?>">
			...
			<td align="left">
				...
				if ( $row->checked_out && ( $row->checked_out != $this->user->get('id') ) ) {
					echo htmlspecialchars($row->geozone_name, ENT_QUOTES, 'UTF-8');
				} else {
				?>
					<a href="<?php echo $link; ?>">
						<?php echo @htmlspecialchars($row->geozone_name, ENT_QUOTES, 'UTF-8'); ?>
					</a>
				<?php
				}
				?>
			...
			<td align="left">
				<?php
				if ( $row->checked_out && ( $row->checked_out != $this->user->get('id') ) ) {
					echo htmlspecialchars($row->typetax_name, ENT_QUOTES, 'UTF-8');
				} else {
				?>
					<a href="<?php echo $link; ?>">
						<?php echo @htmlspecialchars($row->typetax_name, ENT_QUOTES, 'UTF-8'); ?>
					</a>
				<?php
				}
				?>
			...
			<td align="left">
				<?php
				if ( $row->checked_out && ( $row->checked_out != $this->user->get('id') ) ) {
					echo number_format($row->tax_rate, 2, '.', ','). ' %';
				} else {
				?>
					<a href="<?php echo $link; ?>">
