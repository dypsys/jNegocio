...
class jNegocioModelManufacturers extends jFWModelCRUD 
{
	function _buildQueryWhere(&$query)
    {
		...
		if ($filter) {
			...
			$where = array();
			$where[] = '(LOWER(tbl.manufacturer_id) LIKE '.$this->_db->Quote($key).')';
        ...
        if ($filter_id) {
       		$query->where( 'tbl.manufacturer_id ='. $filter_id, 'AND' );
       	}
    }
