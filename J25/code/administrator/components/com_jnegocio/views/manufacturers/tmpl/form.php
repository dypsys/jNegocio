...
<script type="text/javascript">
    ...
    {
        if (task === 'cancel' || document.formvalidator.isValid(document.id('manufacturers-form'))) {
            Joomla.submitform(task, document.getElementById('manufacturers-form'));
        } else {
            ...
            foreach ($this->languages as $lang) {
                ...
                if ($('name_<?php print $lang->language; ?>').hasClass('invalid')) {
                    msg += '\n\n\t* <?= @JText::_('COM_JNEGOCIO_MANUFACTURER_NAME_LABEL'); ?> <?= @JText::_('COM_JNEGOCIO_ITEM_IS_INVALID'); ?>';
                }
            <?php } ?>
            alert(msg);
            return false;
...
<form action="<?= @JRoute::_($this->action); ?>" method="post" id="manufacturers-form" name="adminForm" class="form-validate" enctype="multipart/form-data" >
        ...
        <table width='100%'><tr><td>
            ...
            foreach ($this->languages as $lang) {
                ...
                <fieldset class="adminform">
                    ...
                    <li>
                        <label id="name_<?php print $lang->language; ?>-lbl" class="hasTip required" title="<?= @JText::_('COM_JNEGOCIO_MANUFACTURER_NAME_LABEL'); ?>::<?= @JText::_('COM_JNEGOCIO_MANUFACTURER_NAME_DESC'); ?>" for="name_<?php print $lang->language; ?>">
                            <?= @JText::_('COM_JNEGOCIO_MANUFACTURER_NAME_LABEL'); ?> <?php if ($this->multilang) print "(" . $lang->lang . ")"; ?>
