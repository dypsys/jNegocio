    ...
    <table class="adminlist">
        <thead>
            <tr>
                ...
                <th class="title"><?php echo jFWGrid::jFWsort('COM_JNEGOCIO_GEOZONE_NAME_LABEL', 'tbl.name', @$state->filter_order_Dir, @$state->filter_order); ?></th>
                <th class="title"><?php echo JText::_('COM_JNEGOCIO_GEOZONE_ASSIGNED_ZONE_LABEL'); ?></th>
...
for ($i = 0, $n = count($this->rows); $i < $n; $i++) {
    ...
    $link_zones = 'index.php?option=' . jFWBase::getComponentName() . '&controller=zones&view=zones&layout=select&tmpl=component&filter_geozoneid=' . $row->$tbl_key;
    ...
    if ($row->checked_out && ( $row->checked_out != $this->user->get('id') )) {
        ...
        ?>
                    ...
                    <td align="left"><?php echo @$row->country_name; ?>
    <?php echo JText::_('COM_JNEGOCIO_GEOZONE_ZONES_ASSIGNED') ?>:
                        <?php
                        $options = array('site' => 'admin', 'type' => 'components', 'ext' => 'com_jnegocio');
                        $model = jFWBase::getClass('jNegocioModelGeozonerelations', 'models.geozonerelations', $options);
                        $model->emptyState();
                        $model->setState('filter_geozoneid', $row->$tbl_key);
                        echo $model->getTotal();
                        ?>
                        [<?php echo jFWUrl::popup(@$link_zones, JText::_("COM_JNEGOCIO_SELECT_ZONES"), array('update' => true)); ?>]
                    </td>
