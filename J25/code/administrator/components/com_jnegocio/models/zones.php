...
class jNegocioModelZones extends jFWModelCRUD 
{
	function _buildQueryWhere(&$query)
    {
		...
		if ($filter) {
			...
			$where = array();
			$where[] = '(LOWER(tbl.zone_id) LIKE '.$this->_db->Quote($key).')';
			$where[] = '(LOWER(tbl.name) LIKE '.$this->_db->Quote($key).')';
			$where[] = '(LOWER(tbl.code) LIKE '.$this->_db->Quote($key).')';
        ...
        if ($filter_id) {
       		$query->where( 'tbl.zone_id ='. $filter_id, 'AND' );
       	}

       	if ($filter_countryid) {
       		$query->where( 'tbl.country_id ='. $filter_countryid, 'AND' );
       	}       	
    }
    ...
    {    	
		...
		$query->select( 'ctr.`'.$this->_lang->getField('name') . '` AS country_name' );
		
		$filter_geozoneid = $this->getState('filter_geozoneid');
		if ( intval($filter_geozoneid)>=1 ) {
			$query->select('IF(zr.geozonerelation_id, 1, 0) AS geozone_selected');
		}
    }
    ...
    {
    	$query->join('LEFT','#__'.jFWBase::getTablePrefix().'countries AS ctr ON ctr.country_id = tbl.country_id');
    	$filter_geozoneid = $this->getState('filter_geozoneid');
    	if ( intval($filter_geozoneid)>=1 ) {
        	$query->join('LEFT', '#__'.jFWBase::getTablePrefix().'geozonerelations AS zr ON (zr.zone_id = tbl.zone_id AND zr.geozone_id = '.$filter_geozoneid.')');
        }
    }
