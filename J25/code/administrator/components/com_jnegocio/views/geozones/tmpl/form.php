...
<script type="text/javascript">
	...
	{
		if (task == 'cancel' || document.formvalidator.isValid(document.id('geozones-form'))) {
			Joomla.submitform(task, document.getElementById('geozones-form'));
		} else {
			var msg = '<?php echo $this->escape(JText::_('JGLOBAL_VALIDATION_FORM_FAILED'));?>';
			if($('name').hasClass('invalid')){msg += '\n\n\t* <?= @JText::_( 'COM_JNEGOCIO_GEOZONE_NAME_LABEL' ); ?> <?= @JText::_( 'COM_JNEGOCIO_IS_INVALID' );?>';}
			alert(msg);
			return false;		
		}
	}
...
<form action="<?= @JRoute::_($this->action); ?>" method="post" id="geozones-form" name="adminForm" class="form-validate" >
	<div class="width-60 fltlft">
		<fieldset class="adminform">
			<legend><?php echo empty($this->item->$tbl_key) ? JText::_('COM_JNEGOCIO_NEW_ITEM') : JText::_('COM_JNEGOCIO_EDIT_ITEM'); ?></legend>
			<ul class="adminformlist">
				<li>
					<label id="name-lbl" class="hasTip required" title="<?= @JText::_( 'COM_JNEGOCIO_GEOZONE_NAME_LABEL' ); ?>::<?= @JText::_( 'COM_JNEGOCIO_GEOZONE_NAME_DESC' ); ?>" for="name">
