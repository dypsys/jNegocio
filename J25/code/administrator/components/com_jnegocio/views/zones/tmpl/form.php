...
<script type="text/javascript">
	...
	{
		if (task == 'cancel' || document.formvalidator.isValid(document.id('zones-form'))) {
			Joomla.submitform(task, document.getElementById('zones-form'));
		} else {
			var msg = '<?php echo $this->escape(JText::_('JGLOBAL_VALIDATION_FORM_FAILED'));?>';
			if($('name').hasClass('invalid')){msg += '\n\n\t* <?= @JText::_( 'COM_JNEGOCIO_ZONE_NAME_LABEL' ); ?> <?= @JText::_( 'COM_JNEGOCIO_IS_INVALID' );?>';}
			alert(msg);
			return false;		
		}
	}
...
<form action="<?= @JRoute::_($this->action); ?>" method="post" id="zones-form" name="adminForm" class="form-validate" >
	<div class="width-60 fltlft">
		<fieldset class="adminform">
			<legend><?php echo empty($this->item->$tbl_key) ? JText::_('COM_JNEGOCIO_NEW_ITEM') : JText::_('COM_JNEGOCIO_EDIT_ITEM'); ?></legend>
			<ul class="adminformlist">
				<li>
					<label id="name-lbl" class="hasTip required" title="<?= @JText::_( 'COM_JNEGOCIO_ZONE_NAME_LABEL' ); ?>::<?= @JText::_( 'COM_JNEGOCIO_ZONE_NAME_DESC' ); ?>" for="name">
						<?= @JText::_( 'COM_JNEGOCIO_ZONE_NAME_LABEL' ); ?>
						<span class="star">&nbsp;*</span>
					</label>
					<input class="inputbox required" type="text" name="name" id="name" value="<?= @$this->row->name; ?>" size="30" maxlength="250" />					
				</li>
			   	<li>
			   		<label id="code-lbl" class="hasTip" title="<?= @JText::_( 'COM_JNEGOCIO_ZONE_CODE_LABEL' ); ?>::<?= @JText::_( 'COM_JNEGOCIO_ZONE_CODE_DESC' ); ?>" for="code">
						<?= @JText::_( 'COM_JNEGOCIO_ZONE_CODE_LABEL' ); ?>
					</label>
					<input class="text_area" type="text" name="code" id="code" value="<?= @$this->row->code; ?>" size="5" maxlength="2" />
			   	</li>
			   	<li>
					...
					<?= HelperSelect::countries( @$this->row->country_id, 'country_id', '', 'country_id', false, true );?>
			   	</li>
			</ul>
		</fieldset>
	</div>
